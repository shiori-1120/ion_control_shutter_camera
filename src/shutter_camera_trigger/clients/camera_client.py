...
@dataclass
class CameraClient:
    """Thin client for request/response over camera worker queues.
    ...
    def request(self, cmd: dict, *, timeout: float = 5.0) -> dict:
        ...
        with self._lock:
            ...
            if not isinstance(resp, dict):
                raise RuntimeError(f"Invalid camera response: {resp!r}")
            if not resp.get("ok"):
                raise RuntimeError(resp.get("error", "Camera error"))
            return resp
